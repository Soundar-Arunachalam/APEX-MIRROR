...
import com.pspswitch.tpapegress.model.event.BalanceInquiryEvent;
import com.pspswitch.tpapegress.model.event.EventType;
import com.pspswitch.tpapegress.model.event.SwitchCompletedEvent;
import com.pspswitch.tpapegress.model.payload.BalanceInquiryWebhookPayload;
...
/**
 * Handles BALANCE_INQUIRY events.
 * Dispatches payload built for the balance inquiry sequence to the target TPAP URL via webhook client.
 *
...
@Component
@Slf4j
public class BalanceInquiryHandler implements WebhookEventHandler {
     ...
     */
    public BalanceInquiryHandler(WebhookConfigRepository configRepo,
    ...
    public EventType supportedType() {
        return EventType.BALANCE_INQUIRY;
    }
    ...
    private WebhookPayload buildPayload(SwitchCompletedEvent event) {
        BalanceInquiryEvent data = objectMapper.convertValue(event.getPayload(), BalanceInquiryEvent.class);
        return BalanceInquiryWebhookPayload.builder()
                .eventId(event.getEventId())
                ...
                .inquiryStatus(data.getInquiryStatus())
                .build();
    }
}
